...
public:
	...
	void FinishProcessing1(CommMessage& obj, Socket& socket_);
	void SendFileListing(CommMessage& obj, std::string str);
	void FileListHandler(CommMessage& obj);
	void FileDownloadHandler(CommMessage& obj, std::string str);
	void split(const std::string& string, const std::string& delimiter, std::vector<std::string>& tokens);

	std::vector<std::string>& GetFileList(){ return this->fileList; }
	void Clear(){ this->fileList.clear(); }
	void DisplayFileList();
	ClientHandler() = default;
	ClientHandler(BlockingQueue <std::string>* blockQ) :blockQ(blockQ){}

	void SendTextSearchList(CommMessage& obj, std::string str);
	void SendFileSearchList(CommMessage& obj, std::string str);

private:
	QueueHandler objQueue;
...
{
public:
	void Connect(size_t port, ClientHandler& cp);
	void ConnectServer(size_t port);
};
